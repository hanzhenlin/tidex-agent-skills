...
面向开源生态设计的多供应商、多模型工业级图像生成工作站。支持接入任意兼容 API 接口与网关，提供供应商命名路由、严格模型配套校验，以及「直读 > 代理 > URL」三级零故障交付流水线。

---

## 🎯 官方实际存在模型池与供应商类型

本技能严格按照实际存在的官方生图模型配置，杜绝虚构与非官方别名：

| 供应商类型 (`type`) | 实际可用生图模型 (`models`) | 专长场景 | 协议通道 |
|---|---|---|---|
| **`grok`** | `grok-imagine-image`<br>`grok-imagine-image-quality`<br>`grok-imagine` | 电影级抓拍、前卫超现实、赛博朋克、机械钟表、野生动物细节 | `/v1/images/generations` |
| **`gemini`** | `gemini-3.1-flash-image` | 极致复杂英文文字排版、3D立体穿出海报、禅意摄影、15秒闪电交付 | `/v1/chat/completions` (SSE 流式) |
...
- **`proxy_url`**：唯一代理配置。自动智能适配（本地代理端口 `http://127.0.0.1:7897` 或反代前缀镜像）；
...
2. **仅指定供应商 (`-p`)**：
   ```bash
   python3 scripts/gen_image.py "水墨禅意白瓷茶杯" -p gemini-provider
   ```
...
4. **严格配套拦截（防跨站乱调）**：
   ```bash
   python3 scripts/gen_image.py "测试" -p gemini-provider -m grok-imagine-image
   ```
   *立即毫秒级报错拦截：`供应商 gemini-provider 不支持模型 grok-imagine-image`，绝不发出错误请求！*
...
| 参数 | 说明 |
|---|---|
| `-p, --provider` | 指定供应商名字（如 `grok-provider`, `gemini-provider`） |
